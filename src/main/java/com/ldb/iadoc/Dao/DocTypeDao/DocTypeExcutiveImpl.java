...
@Service
public class DocTypeExcutiveImpl implements DocTypeExcutvieDao {
    @Autowired
    ...
    public int SaveDocTypeExcutive(DocTypeReq docTypeReq) {
       SQL="insert into doc_type (DOC_DESC,DOC_DESC_LAO,type) values (?,?,'2')";
    ...
    public List<DocType> getDocumentTypeExcutive(DocTypeReq docTypeReq) {
        if(docTypeReq.getDocType() == null || docTypeReq.getDocType() == ""){
            SQL="select * from doc_type where type='2' order by DOC_TYPE asc";
        }else {
            SQL="select * from doc_type where  type='2' and DOC_TYPE='"+docTypeReq.getDocType()+"' order by DOC_TYPE asc";
        }
